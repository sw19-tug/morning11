...
public class EraserTest {
    ...
    public void testDraw() {
        Eraser eraserClass = new Eraser(canvas, paint, mockedBackgroundColor);        // TEST ACTION_DOWN
        float xy = 10.f;
        ...
        when(mockEvent.getX()).thenReturn(xy);
        when(mockEvent.getY()).thenReturn(xy);
        eraserClass.draw(mockEvent);        // TEST ACTION_MOVE 2
        xy = 12.f;
        when(mockEvent.getAction()).thenReturn(MotionEvent.ACTION_MOVE);
        when(mockEvent.getX()).thenReturn(xy);
        when(mockEvent.getY()).thenReturn(xy);
        eraserClass.draw(mockEvent);        // TEST ACTION_UP
        xy = 13.f;
        when(mockEvent.getAction()).thenReturn(MotionEvent.ACTION_UP);
        when(mockEvent.getX()).thenReturn(xy);
        when(mockEvent.getY()).thenReturn(xy);
        eraserClass.draw(mockEvent);        // this should fire 2 times
